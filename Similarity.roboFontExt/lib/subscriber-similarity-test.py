...
class DemoWindowController(Subscriber, ezui.WindowController):
    ...
    def build(self):
        ...
        * HorizontalStack
        ...
        descriptionData = dict(
            glyphNameTextBox=dict(
                width=500,
                text=f"<current GlyphName>"
            ),
            valueSlider=dict(
                minValue=0.1,
                value=self.factor,
    ...
    def started(self):
        addObserver(self, "observerCallbackcurrentGlyphChanged", "currentGlyphChanged")
    ...
    def observerCallbackcurrentGlyphChanged(self,notification):
        textBox = self.w.getItem("glyphNameTextBox")
        item = notification['glyph']
        if item is None:
            textBox.set(f"No current selected")
        else:
            self.glyphName = notification['glyph'].name
            textBox.set(f"{self.glyphName}")
